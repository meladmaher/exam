          ...
          <div key={idx} className={`modern-card p-5 sm:p-10 border-r-4 sm:border-r-8 transition-all ${item.isCorrect ? 'border-green-500' : 'border-red-500'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="p-4 sm:p-5 bg-slate-50 dark:bg-slate-800 rounded-2xl">
                ...
                <p className={`text-base sm:text-lg font-black whitespace-pre-line leading-relaxed ${item.isCorrect ? 'text-green-600' : 'text-red-500'}`}>{item.userAnswer || 'لم يتم الحل'}</p>
              </div>
              {!item.isCorrect && (
                <div className="p-4 sm:p-5 bg-indigo-50 dark:bg-indigo-900/30 rounded-2xl">
                  <span className="text-[10px] font-black text-slate-400 uppercase block mb-1 sm:mb-2">الإجابة الصحيحة النموذجية:</span>
                  <p className="text-base sm:text-lg font-black text-primary whitespace-pre-line leading-relaxed">
      ...
      <main className="flex-grow py-6 sm:py-12 px-3 sm:px-4 max-w-7xl mx-auto w-full">
        ...
        {activeView === 'exam' && activeExam && (
          <ExamPlayer 
            ...
            onCancel={() => { storageService.clearSession(); setActiveExam(null); setActiveView('home'); }} 
          />
        )}
...
const ExamPlayer: React.FC<{ exam: Exam, lang: 'ar'|'en', onFinish: (res: any) => void, onCancel: () => void }> = ({ exam, lang, onFinish, onCancel }) => {
  ...
  const validate = async (val: string) => {
    ...
    let isCorrect = q.type === 'fill-in-the-blank' 
      ? await aiService.evaluateAnswer(val, q.correctAnswer, q.questionText[lang])
  ...
  return (
    <div className="max-w-4xl mx-auto p-1 sm:p-4 animate-fade-in text-start flex flex-col">
      <div className="flex justify-between items-center mb-6 sm:mb-10 bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 gap-3">
        <button onClick={() => setShowExitConfirm(true)} className="w-10 h-10 sm:w-14 sm:h-14 bg-red-50 text-red-500 rounded-xl sm:rounded-2xl flex items-center justify-center font-black shrink-0 hover:bg-red-100 transition-all">✕</button>
      ...
      <div className={`flex-1 modern-card p-6 sm:p-14 mb-8 sm:mb-10 transition-all duration-500 border-2 ${!session.feedback[q.id]?.isChecked ? 'active-question-glow border-primary/30' : 'border-slate-100 dark:border-slate-800'}`}>
        ...
        <div className="space-y-4 sm:space-y-6">
          {q.type !== 'fill-in-the-blank' ? q.options?.map((opt, idx) => {
            ...
            <div className="space-y-6 sm:space-y-10 text-center max-w-2xl mx-auto">
              <textarea 
                ...
                rows={5}
                className="w-full p-4 sm:p-6 text-base sm:text-2xl font-bold rounded-2xl sm:rounded-[2.5rem] border-4 text-start dark:bg-slate-950 focus:border-primary outline-none transition-all shadow-inner resize-y min-h-[160px] leading-relaxed" 
                placeholder="اكتب إجابتك هنا بالتفصيل (يمكنك الضغط على Enter للانتقال لسطر جديد)..." 
                autoFocus 
              />
          ...
          {session.feedback[q.id]?.isChecked && !session.feedback[q.id].isCorrect && (
            <div className="p-6 sm:p-10 bg-indigo-50 dark:bg-indigo-900/10 rounded-2xl sm:rounded-[3rem] border-2 sm:border-4 border-indigo-100 mt-6 sm:mt-10 animate-fade-in shadow-inner text-start">
              <span className="text-[10px] font-black text-indigo-400 uppercase mb-2 sm:mb-4 block tracking-widest">{t.correctAnswer}:</span>
              <p className="text-base sm:text-2xl font-black text-indigo-700 dark:text-indigo-300 leading-relaxed whitespace-pre-line">{q.type === 'fill-in-the-blank' ? q.correctAnswer : q.options?.[Number(q.correctAnswer)]?.[lang]}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showExitConfirm && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[3000] flex items-center justify-center p-4 animate-fade-in text-center">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] p-6 sm:p-10 shadow-2xl border-2 border-red-500/20">
            <div className="text-5xl mb-4">⚠️</div>
            <h3 className="text-xl sm:text-2xl font-black mb-3">هل تريد إنهاء الاختبار؟</h3>
            <p className="text-slate-500 dark:text-slate-400 font-bold mb-6 text-sm sm:text-base leading-relaxed">
              سيتم إلغاء المحاولة الحالية والعودة للصفحة الرئيسية.
            </p>
            <div className="flex gap-3">
              <button 
                onClick={() => setShowExitConfirm(false)} 
                className="flex-1 py-4 bg-slate-100 dark:bg-slate-800 rounded-2xl font-black text-slate-700 dark:text-slate-300 hover:bg-slate-200 transition-all"
              >
                متابعة الاختبار
              </button>
              <button 
                ...
                className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-black shadow-lg hover:bg-red-600 transition-all"
              >
                نعم، الخروج
              </button>
            </div>
