...
import { SMARTPHONE_APPS_OBJECTIVE_EXAM, SMARTPHONE_APPS_ESSAY_EXAM, SMARTPHONE_APPS_EXAM } from '../constants';
...
const ONE_TIME_RESET_KEY = 'ai_exam_github_v5_reset';

// Perform a one-time localStorage reset when deployed/opened to reset previous cached exams cleanly
if (typeof window !== 'undefined') {
  try {
    if (localStorage.getItem(ONE_TIME_RESET_KEY) !== 'done_v5') {
      localStorage.clear();
      localStorage.setItem(ONE_TIME_RESET_KEY, 'done_v5');
    }
  } catch (e) {
    console.error('Failed to perform one-time localStorage reset:', e);
  }
}
...
export const storageService = {
  ...
  getCustomExams: (): Exam[] => {
    ...
    const savedExams: Exam[] = saved ? JSON.parse(saved) : [];
    
    const defaults = [
      SMARTPHONE_APPS_OBJECTIVE_EXAM,
      SMARTPHONE_APPS_ESSAY_EXAM,
      SMARTPHONE_APPS_EXAM
    ];

    const examMap = new Map<string, Exam>();
    savedExams.forEach(e => examMap.set(e.id, e));
    defaults.forEach(e => examMap.set(e.id, e));
