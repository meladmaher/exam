...
import { ELECTRONIC_EXAMS_MCQ, ELECTRONIC_EXAMS_ESSAY } from '../constants';
...
const ONE_TIME_RESET_KEY = 'ai_exam_v3_reset_flag';

// Perform a one-time localStorage reset for existing users to reflect new exam structure & updates
if (typeof window !== 'undefined') {
  try {
    if (localStorage.getItem(ONE_TIME_RESET_KEY) !== 'done_v3') {
      localStorage.clear();
      localStorage.setItem(ONE_TIME_RESET_KEY, 'done_v3');
    }
  } catch (e) {
...
export const storageService = {
  ...
  getCustomExams: (): Exam[] => {
    ...
    const defaults = [
      ELECTRONIC_EXAMS_MCQ,
      ELECTRONIC_EXAMS_ESSAY
    ];
